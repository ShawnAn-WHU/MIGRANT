...
DOTA_v2_0_remove_obj = "/home/anxiao/Datasets/MIGRANT/DOTA-v2_0/label_remove_2_to_10.json"
DIOR_R_remove_obj = "/home/anxiao/Datasets/MIGRANT/DIOR-R/label_remove_2_to_10.json"
DIOR_R_remove_vehicle = "/home/anxiao/Datasets/MIGRANT/DIOR-R/label_remove_2_to_4_vehicle.json"
...
for item in tqdm(dg_data):
    ...
    if dg_type == "compare_dg":
        ...
        qa_item[0]["content"] = query_prefix + random.choice(dg_query.dg_compare) + " " + random.choice(dg_query.dg_bbox).replace("bounding box", f"{bbox_type} bounding box")
        ...
        if compare == "appear":
            qa_item[1]["content"] = random.choice(dg_query.dg_describe_answer_appear).replace(
                "<object>", 
                ", ".join(f"{count} more {name}" for name, count in object_counts.items())
            )
        else:
            qa_item[1]["content"] = random.choice(dg_query.dg_describe_answer_disappear).replace(
                "<object>", 
                ", ".join(f"{count} {name}" for name, count in object_counts.items())
            )
        qa["messages"].extend(qa_item)
        qa_item = copy.deepcopy(constants.QWEN2_VL_QA_FORMAT)
        qa_item[0]["content"] = random.choice(dg_query.dg_bbox).replace("bounding box", f"{bbox_type} bounding box")
        qa_item[1]["content"] = "".join(
            f"<|object_ref_start|>{object_item['object_name']}<|object_ref_end|><|box_start|>{format_bbox(bbox_type, object_item)}<|box_end|>"
            for object_item in objects
        )
