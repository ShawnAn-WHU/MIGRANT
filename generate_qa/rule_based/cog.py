...
def format_bbox(bbox_type, bbox_item):
    ...
    coords = json.loads(bbox_item["objects"][0][bbox_type])
    if bbox_type == "hbb":
    ...
    while len(image_item_dict[category]) >= 4:
        ...
        if cog_type == "text_with_1st":
            for i in range(1, len(images) + 1):
                ...
                bbox = format_bbox(bbox_type, selected_item[i - 1])
                qa_item[1][
                    "content"
                ] = f"<|object_ref_start|>{category}<|object_ref_end|><|box_start|>{bbox}<|box_end|>"
            ...
            qa_item[1]["content"] = "".join(
                f"Image-{i+1}: <|object_ref_start|>{category}<|object_ref_end|><|box_start|>{format_bbox(bbox_type, item)}<|box_end|>"
